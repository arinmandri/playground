 ...
 * 새 항목을 만드는 경우
 * - newFile: 새로 선택한 파일
 * - preview: null --(파일 선택)--> FileReader를 이용하여 새로 선택한 파일에서 만들어진 미리보기
 ...
 * - newFile: 새로 선택한 파일
 * - preview: 기존 항목의 URL --(파일 선택)--> FileReader를 이용하여 새로 선택한 파일에서 만들어진 미리보기
 * - fieldValue: 기존 항목의 URL --(파일 업로드 API 호출)--> 첨부파일의 임시파일id
 */
export interface FileAndPreview {
  newFile: File | null;// 첨부 파일
  preview: string | null;// 미리보기 (기존 항목을 불러온 경우 기존 항목의 URL, 새 항목을 만들거나 기존 항목을 수정하는 경우 FileReader를 이용하여 새로 선택한 파일에서 만들어진 미리보기)
